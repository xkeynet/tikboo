...
// =========================================================
// === SUPABASE / DYNAMIC FEED ===
// =========================================================

// Browser-safe values only.
// NEVER use Supabase secret key or service_role key here.
...
async function supabaseFetch(pathname, params = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `[Tikboo] Supabase ${pathname} request failed: ` +
...
async function loadDynamicPlaylist() {
  ...
  const playlist = videos.map((video) => {
    ...
    if (!creator) {
      throw new Error(
        `[Tikboo] Creator "${video.creator_handle}" ` +
        `was not found in creators table.`
      );
    }

    if (!video.hls_url) {
      throw new Error(
        `[Tikboo] Ready video "${video.id}" has no hls_url.`
      );
    }

    if (!video.poster_url) {
      throw new Error(
        `[Tikboo] Ready video "${video.id}" has no poster_url.`
      );
    }
  ...
  if (!playlist.length) {
    throw new Error(
      '[Tikboo] Supabase returned zero ready active videos.'
    );
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  // =========================================================
  // === Load production feed from Supabase ===
  // =========================================================
  try {
    PLAYLIST = await loadDynamicPlaylist();

    console.log(
      `[Tikboo] Supabase feed loaded: ${PLAYLIST.length} videos`
    );
  } catch (err) {
    console.error(
      '[Tikboo] Failed to load Supabase feed:',
      err
    );
  ...
  let ageGateUnlocked =
    window.TikbooCookieConsent.isUnlocked();
  ...
  function showPlayOverlay(show) {
    ...
    refs.playOverlay.style.opacity = show
      ? '1'
      : '0';
  }
  ...
  function setAllSidesScrubbing(on) {
    getAllSides().forEach((sideEl) => {
      sideEl.classList.toggle(
        'scrubbing',
        !!on
      );

      sideEl.style.opacity = on
        ? '0'
        : '';

      sideEl.style.pointerEvents = on
        ? 'none'
        : '';

      sideEl.style.display = on
        ? 'none'
        : '';

      const layer = sideEl.closest(
        '.twincher-layer'
      );

      const videoMeta = layer?.querySelector(
        '.video-meta'
      );

      if (videoMeta) {
        videoMeta.classList.toggle(
          'scrubbing',
          !!on
        );

        videoMeta.style.opacity = on
          ? '0'
          : '';

        videoMeta.style.pointerEvents = on
          ? 'none'
          : '';

        videoMeta.style.display = on
          ? 'none'
          : '';
      }
    });
  ...
  function setLayerVideoMeta(
    layer,
    item
  ) {
    if (!layer || !item) return;

    const meta = layer.querySelector(
      '.video-meta'
    );

    if (!meta) return;

    const creatorEl = meta.querySelector(
      '.video-meta-creator'
    );

    const captionEl = meta.querySelector(
      '.video-meta-caption'
    );

    const followEl = meta.querySelector(
      '.video-meta-follow'
    );

    const avatarEl =
      layer.querySelector('.avatar');
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      const isOverflowing =
        captionEl.scrollWidth >
        captionEl.clientWidth + 1;
  ...
  function setSeekActive(on) {
    ...
    if (seekTime) {
      seekTime.classList.toggle(
        'is-active',
        !!on
      );
    }

    setAllSidesScrubbing(
      !!on
    );
  ...
  function seekToClientX(
    ...
    const r =
      seekPill.getBoundingClientRect();
  ...
  function watchFirstRenderedFrame(
    video,
    poster
  ) {
...
(() => {
  ...
  targets.forEach((img) => {
    ...
    img.style.webkitTapHighlightColor =
      'transparent';
